...
export const CoverflowCarousel = React.forwardRef(function CoverflowCarousel(
  {
    slides = [],
    rotate = 44,
    depth = 0.6,
    ...
    falloff = 0.56,
    fade = 0.1,
    cardWidth = "clamp(240px, 32vw, 360px)",
    gap = 0.05,
    loop = true,
    showCaption = false,
  ...
  const clickedCardIndexRef = React.useRef(null);
  const pointerDownPosRef = React.useRef({ x: 0, y: 0, time: 0 });
  ...
  // Dedicated click handler ensuring instant wider view on laptop, PC & mobile
  const handleCardAction = React.useCallback(
    (idx) => {
      if (idx === null || idx === undefined || isNaN(idx) || idx < 0 || idx >= count) return;
  ...
  const onPointerDown = (event) => {
    ...
    // Capture which slide card was pressed
    const targetCard = event.target.closest("[data-slide-index]");
    const clickedIdx = targetCard ? parseInt(targetCard.getAttribute("data-slide-index"), 10) : null;
    clickedCardIndexRef.current = clickedIdx;
    pointerDownPosRef.current = { x: event.clientX, y: event.clientY, time: performance.now() };
  ...
  const endDrag = (event) => {
    ...
    // Desktop/Laptop Click: Released with minimal movement (< 8px)
    if (
      dist < 8 &&
      duration < 650 &&
      clickedCardIndexRef.current !== null &&
      !isNaN(clickedCardIndexRef.current)
    ) {
      const idx = clickedCardIndexRef.current;
      ...
      <div className="relative">
        <div
          ...
          onClick={(e) => {
            ...
            const targetCard = e.target.closest("[data-slide-index]");
            if (targetCard) {
              const idx = parseInt(targetCard.getAttribute("data-slide-index"), 10);
              handleCardAction(idx);
            }
          }}
          onKeyDown={(event) => {
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-xl will-change-transform cursor-pointer transition-shadow hover:shadow-2xl select-none",
                  cardClassName,
                )}
